...
export const selectTransactionsByDate = createSelector(
  ...
  (transactions, dateQuery) => {
    ...
    const firstDateYear = (isRange) ? parseInt(dateQuery.split('&')[0].slice(0,4)) :  parseInt(dateQuery.slice(0,4));
    const firstDateMonth = (isRange) ? parseInt(dateQuery.split('&')[0].slice(4,6)) : parseInt(dateQuery.slice(4,6));
    const firstDateDay = (isRange) ? parseInt(dateQuery.split('&')[0].slice(6)) : parseInt(dateQuery.slice(6));

    const secondDateYear = (isRange) ? parseInt(dateQuery.split('&')[1].slice(0,4)) : null;
    const secondDateMonth = (isRange) ? parseInt(dateQuery.split('&')[1].slice(4,6)) : null;
    const secondDateDay = (isRange) ? parseInt(dateQuery.split('&')[1].slice(6)) : null;
